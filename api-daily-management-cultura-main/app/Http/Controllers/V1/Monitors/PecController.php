...
class PecController extends Controller
{
    ...
    public function getByRangeActivityDate(Request $request)
    {
        ...
        $results = $this->pecRepository->getByRangeActivityDate($filter, $initDate, $lastDate);
        return response()->json(['items' => $results]);
    }

    public function getPecsCreatedBy(Request $request, $created_by)
    {
        try {
            $result = $this->pecRepository->getPecsCreatedBy($created_by);
            if (empty($result)) {
                return $this->createResponse($result, 'No se encontró pec', 404);
            }
            return $this->createResponse($result, 'La pec fue encontrada');
