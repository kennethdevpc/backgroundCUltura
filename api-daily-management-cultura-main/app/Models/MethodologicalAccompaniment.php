...
class MethodologicalAccompaniment extends Model
{

    use HasFactory,SoftDeletes;
    protected $table = 'methodological_accompaniments';
    ...
    public function nac(){
        return $this->belongsTo(Nac::class,'id');
    }
    public function nacMetho(){
    ...
    {
        return $this->belongsToMany(Profile::class, 'methodological_accompaniment_user', 'metho_accompaniment_id', 'assistant_id')
        ...
        if (request()->filled('search_field') && request()->filled('search_value')) {
            ...
            $validSearchFields = [
                ...
                'nac.name' => function ($query) use ($searchValue) {
                    $query->whereHas('nacMetho', function ($query) use ($searchValue) {
                        $query->where('nacs.name', 'like', '%' . $searchValue . '%');
                    });
                },
                'aspects' => function ($query) use ($searchValue) {
                    $query->where('aspects', 'like', '%' . $searchValue . '%');
                },
                /* 'roles_associate' => function ($query) use ($searchValue) {
