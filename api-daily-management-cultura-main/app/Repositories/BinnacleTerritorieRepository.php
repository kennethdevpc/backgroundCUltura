...
class BinnacleTerritorieRepository
{
    ...
    public function getAll()
    {
        ...
        $query = $this->model->query()->select(['binnacle_territories.*'])
            ->join('users', 'users.id', '=', 'created_by')
            ->join('role_user', 'role_user.user_id', '=', 'users.id')
            ->join('roles', 'roles.id', '=', 'role_user.role_id');

        if (config('roles.subdireccion') == $rol_id || config('roles.direccion') == $rol_id || config('roles.secretaria_cultural') == $rol_id || config('roles.root') == $rol_id || config('roles.super_root') == $rol_id) {
            $binnacles =   $query
                ->whereIn('roles.slug', ['coordinador_supervision', 'coordinador_psicosocial', 'coordinador_seguimiento', 'coordinador_administrativo', 'coordinador_metodologico'])
                ->orderByRaw("FIELD(roles.slug,'coordinador_supervision','coordinador_psicosocial', 'coordinador_seguimiento', 'coordinador_administrativo', 'coordinador_metodologico') ASC")
                ->get();
        }
        ...
        session()->forget('count_page_binnacleTerritorie');
        session()->put('count_page_binnacleTerritorie', ceil($query->get()->count()/$paginate));

        return new BinnacleTerritorieCollection($query->simplePaginate($paginate));
    }

    public function getAllByUserLogged()
    {
        $binnacles = $this->model->where('created_by', '=', Auth::id())->orderByRaw("FIELD(status,'REC','ENREV','REV','APRO') ASC")
            ->orderBy('id', 'DESC')->get();
    ...
    public function create(Request $request)
    {
        ...
        DB::update(DB::RAW("UPDATE binnacle_territories SET consecutive = CONCAT('BV', id) WHERE id=$binnacleTerritorie->id"));
    ...
    public function rolesCustom($id)
    {
        if ($id == 0) return null;
        $nac = Nac::find($id);
        $profile = Profile::where('nac_id', $nac->id)
            ->where(function ($query) {
                $query->where('role_id', config('roles.instructor'))
                    ->orWhere('role_id',  config('roles.monitor'));
            })
            ->get();
