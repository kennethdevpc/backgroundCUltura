...
class DialogueTableRepository
{
    ...
    public function getAll()
    {
        ...
        if ($rol_id == config('roles.gestor')) {
            $dialogueTables =  $query->where('user_id', $user_id);
        }
        if ($rol_id == config('roles.apoyo_metodologico')) {
            $dialogueTables =  $query->where('user_method_support_id', $user_id);
        }

        if ($rol_id == config('roles.root') || $rol_id == config('roles.super_root')) {
            $dialogueTables =  $query;
        }
        ...
        if ($rol_id == config('roles.direccion') || $rol_id == config('roles.secretaria_cultural') || $rol_id == config('roles.coordinador_supervision') || $rol_id == config('roles.apoyo_supervision')) {
            $dialogueTables = $query->whereHas('user', function ($query) {
        ...
        session()->forget('count_page_dialogue_tables');
        session()->put('count_page_dialogue_tables', ceil($query->count() / $paginate));

        $results = new DialogueTableCollection($query->simplePaginate($paginate));
        return $results;
    }
    public function create($request)
    {
        ...
        try {
            ...
            DB::update(DB::RAW("UPDATE dialogue_tables SET consecutive = CONCAT('IMD', id) WHERE id=$dialogueTable->id"));
    ...
    public function getValidate($data, $method)
    {
        $validate = [
            ...
            'workday_description' => 'required|max:3500',
