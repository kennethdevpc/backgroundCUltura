...
class MethodologicalInstructionRepository
{
    ...
    function getAll()
    {
        ...
        $query = $this->methodological_instruction->query()->with(['nac', 'expertise'])->orderByRaw("FIELD(status,'REC','ENREV','REV','APRO') ASC")
        ->orderBy('id', 'DESC');

        $methodological_instructions = [];
        $paginate = config('global.paginate');
        if ($rol_id == config('roles.gestor')) {
            $methodological_instructions =  $query->where('created_by', $user_id);
        }
        if ($rol_id == config('roles.apoyo_metodologico')) {
            $methodological_instructions =  $query->where('user_method_support_id', $user_id);
        }

        if ($rol_id == config('roles.root') || $rol_id == config('roles.super_root')) {
            $methodological_instructions =  $query;
        }
        if($rol_id == config('roles.direccion') || $rol_id == config('roles.secretaria_cultural') || $rol_id == config('roles.coordinador_supervision') || $rol_id == config('roles.apoyo_supervision')){
            $methodological_instructions = $query->whereHas('user', function ($query) {
        ...
        $query = $this->methodological_instruction->scopeFilterByUrl($query);
        session()->forget('count_page_methodological_instructions');
        session()->put('count_page_methodological_instructions', ceil($query->count()/$paginate));
        return new MethodologicalInstructionCollection($query->simplePaginate($paginate));
    }
    ...
    function create(Request $request)
    {
        ...
        try {
            ...
            $id = $this->methodological_instruction->id;
            DB::update(DB::RAW("UPDATE methodological_instructions SET consecutive = CONCAT('AM', id) WHERE id=$id"));
