...
class ParentSchoolRepository
{
    ...
    public function createParentSchool($request)
    {
        ...
        try {
            ...
            DB::update(DB::RAW("UPDATE parent_schools SET consecutive = CONCAT('EP', id) WHERE id=$parent_school->id"));
            if ($save) {
    ...
    public function getAll()
    {
        try {
            ...
            $results = $this->model->orderByRaw("FIELD(status,'REC','ENREV','REV','APRO') ASC");

            if ($rol_id == config('roles.psicosocial')) {
                $results->where('created_by', $user_id);
            }
            /* if ($rol_id == config('roles.coordinador_psicosocial')) {
                $query->where('user_psychoso_coordinator_id', $user_id);
            } */
            if ($rol_id == config('roles.direccion') || $rol_id == config('roles.coordinador_psicosocial') || $rol_id == config('roles.secretaria_cultural') || $rol_id == config('roles.coordinador_supervision') || $rol_id == config('roles.apoyo_supervision')) {
                $results->whereHas('user', function ($query) {
            ...
            $query = $this->model->scopeFilterByUrl($results);

            session()->forget('count_page_parentSchools');
            session()->put('count_page_parentSchools', ceil($results->get()->count()/$paginate));

            return new ParentSchoolCollection($results->simplePaginate($paginate));

            // return new ParentSchoolCollection($results);
        } catch (Exception $ex) {
            report($ex);
