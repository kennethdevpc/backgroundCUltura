...
class PsychopedagogicalLogBookRepository
{
    ...
    public function create($request)
    {
        try {
            ...
            if ($save) {
                ...
                DB::update(DB::RAW("UPDATE psychopedagogical_logbooks SET consecutive = CONCAT('BTP', id) WHERE id=$psychopedagogical_logbook->id"));
            }
    ...
    private function baseQuery()
    {
        $rol_id = $this->getIdRolUserAuth();
        $user_id = $this->getIdUserAuth();
        $query = PsychopedagogicalLogbook::query()->orderByRaw("FIELD(status,'REC','ENREV','REV','APRO') ASC")
            ->orderBy('id', 'DESC');

        $psychopedagogicalLogbooks = [];
        if ($rol_id == config('roles.psicosocial')) {

            $psychopedagogicalLogbooks =  $query->where('psychopedagogical_logbooks.user_id', $user_id)->get();
        }
        ...
        /* if ($rol_id == config('roles.coordinador_psicosocial')) {
            ...
            $psychopedagogicalLogbooks =  $query->where('psychopedagogical_logbooks.user_psychoso_coordinator_id', $user_id)->get();
        } */

        if ($rol_id == config('roles.root') || $rol_id == config('roles.super_root')) {
            $psychopedagogicalLogbooks =  $query->get();
        }
        if ($rol_id == config('roles.direccion')  || $rol_id == config('roles.coordinador_psicosocial') || $rol_id == config('roles.secretaria_cultural') || $rol_id == config('roles.coordinador_supervision') || $rol_id == config('roles.apoyo_supervision')) {
            $psychopedagogicalLogbooks = $query->whereHas('user', function ($query) {
    ...
    public function getAll()
    {
        try {
            ...
            if ($rol_id == config('roles.psicosocial')) {
                $query->where('user_id', $this->getIdUserAuth());
            }
            ...
            session()->forget('count_page_psychopedagogicalLogBook');
            session()->put('count_page_psychopedagogicalLogBook', ceil($query->get()->count()/$paginate));

            return new PsychopedagogicalLogBookCollection($query->simplePaginate($paginate));
