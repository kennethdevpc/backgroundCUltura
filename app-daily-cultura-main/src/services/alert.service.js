...
export default function activityLogService() {
	...
	const get = async (page) => {
		try {
			setLoading(true)
			let response = await axiosInstance.get(`/${apiPath}/alertsPaginate?page=`+page).finally(() => {
				setLoading(false)
			});
	...
	const update = async (id, payload) => {
		...
		try {
			...
			if (response.status == 200) {
				router.push({ name: "alerts.index", params: {} });
			}
		} catch (e) {
			alerts.custom_validation(e.response.data.error ?? e.response.data.message);
		}
	...
	const destroy = async (id) => {
		try {
			...
			if (response.status === 200) await router.push({ name: "alerts.index" });
		} catch (e) {
			alerts.custom_validation(e.response.data.error ?? e.response.data.message);
		}
	};
