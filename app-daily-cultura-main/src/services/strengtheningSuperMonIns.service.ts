...
const module = "strengtheningSuperMonIns";
...
export default function strengtheningSuperMonInsService() {
    const data = reactive({
        all: [],
        one: {},
        no_paginate: [],
    ...
    const update = async (id, payload) => {
        ...
        try {
            ...
            if (response.status >= 200 && response.status <= 300) {
                alerts.update();
                router.push({ name: "strengtheningSuperMonIns.index" });
            }
        } catch (e) {
            alerts.custom_validation(e.response.data.error ?? e.response.data.message);
        }
