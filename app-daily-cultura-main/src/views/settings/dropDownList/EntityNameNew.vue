...
const fetchOne = async () => {
	return await entitynames.getOne('entities', route.params.id)
}
...
const onSubmit = async () => {
	...
	if (valid){
		if (isEditing.value)
			await entitynames.update('entities', entitynames.dataOne.value.id, drop_downs.transpiledData, 'entities')
		else
			await entitynames.create('entities', drop_downs.transpiledData, 'entities')
	}
	else {
		alerts.validation()
	}
