		...
		sections: {
			general: {
				...
				fields: {
					NAC: item.nac.name,
					ROL: rol.value,
					USUARIO: mixins.get_option_label(
						"users_table",
						item.user.id
					),
			...
			development: {
				...
				fields: {
					"OBJETIVOS ESTRATÉGICOS DEL ÁREA":
						item.strategic_objectives_area,
			...
			diagnostic: {
				...
				fields: {
					...
					"PERCEPCIÓN DE LOS PARTICIPANTES FRENTE A LAS ACTIVIDADES DESARROLLADAS POR EL ÁREA":
						item.participants_perception,
					"DIFICULTADES O PROBLEMÁTICAS IDENTIFICADAS":
						item.problems_identified,
					"RECOMENDACIONES Y ACCIONES DE MEJORA PROPUESTAS POR LOS PARTICIPANTES ":
						item.recommendations_actions,
					"PERCEPCIONES/COMENTARIOS/ANÁLISIS FRENTE AL AVANCE DEL PROCESO":
						item.comments_analysis,
				},
			},
			multimedia: {
				title: "Multimedia",
...
const permission_role = is_role("subdireccion")
	? permissions.subdireccion.crud_management()
	: permissions.direccion.crud_management_coordinador();
...
<template>
	<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
		<h2 class="text-lg font-medium mr-auto">
			Reporte visita a territorio de coordinadores
		</h2>
	...
	<div class="intro-y box mt-5 p-5">
		<BaseCrud
			...
			:management_permissions="permission_role"
			label="la bitácora"
