...
<template>
    <FormHeader
        :state="{ consecutive: instance.consecutive, status: instance.status, reject_message: instance.reject_message }">
                {{ (editing) ? `Edición de Informe mensual de supervisión #${services.data.one.id}` : `Informe mensual de supervisión` }}
    </FormHeader>
    <Management class="mt-5" v-if="(editing) ? permissions.monthlyMonitoring.management() : false"
        @send="(payload) => mixins.methods.send_management('monthly_monitoring_reports', services.data.one.id, payload)" />
    <div class="intro-y box mt-5">
        <fieldset :disabled="permissions.monthlyMonitoring.no_edit()">
